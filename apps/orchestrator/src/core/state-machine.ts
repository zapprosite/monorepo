...
import type { WorkflowEvent, WorkflowState, WorkflowStatus } from './types.js';
...
export class WorkflowStateMachine {
	...
	canTransition(event: WorkflowEvent['type']): boolean {
		return VALID_TRANSITIONS.some((t) => t.from === this.state.status && t.event === event);
	}
	...
	transition(event: WorkflowEvent): WorkflowState {
		...
		if (!transition) {
			throw new Error(`Invalid transition: ${this.state.status} + ${eventType}`);
		}

...
change(async (db) => {
  await db.createEnum('public.crm_service_order_status_enum', [
    'Aberta', 'Em Andamento', 'Aguardando Peças', 'Concluída', 'Cancelada',
  ]);
  ...
  await db.createTable(
    ...
    (t) => ({
      ...
      scheduleId: t.uuid().foreignKey('schedules', 'scheduleId', {
        onUpdate: 'RESTRICT',
        onDelete: 'SET NULL',
      }).nullable(),
      unitId: t.uuid().foreignKey('units', 'unitId', {
        onUpdate: 'RESTRICT',
        onDelete: 'SET NULL',
      }).nullable(),
      equipmentId: t.uuid().foreignKey('equipment', 'equipmentId', {
        onUpdate: 'RESTRICT',
        onDelete: 'SET NULL',
      }).nullable(),

...
(() => {
	...
	function generateCodeVerifier() {
		...
		return base64UrlEncode(array).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
	}
	...
	async function generateCodeChallenge(verifier) {
		...
		return base64UrlEncode(hashArray).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
	}

	function base64UrlEncode(array) {
		return btoa(String.fromCharCode.apply(null, array)).replace(/=+$/, '');
	}
	...
	function createHeader(user) {
		...
		logo.innerHTML =
			'<span class="header-logo-icon">\u{1F4D0}</span><span>Obsidian Vault</span><span> — md.zappro.site</span>';
		...
		searchWrapper.innerHTML =
			'<div class="search-wrapper"><span class="search-icon">\u{1F50D}</span><input type="text" class="search-input" placeholder="Buscar arquivos..." id="search-input"></div>';
	...
	function createSidebar(activePath) {
		...
		rootItem.innerHTML =
			'<span class="folder-icon">\u{1F4C1}</span><span class="folder-name">Vault</span>';
		rootItem.onclick = () => {
			navigateToPath('/');
		};
	...
	function createBackButton() {
		...
		btn.style.cssText =
			'margin-bottom: 1rem; padding: 0.5rem 1rem; background: var(--color-card); border: 1px solid var(--color-border); border-radius: 8px; color: var(--color-text); cursor: pointer;';

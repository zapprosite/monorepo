...
import type { RagRetrieveResult } from './types.js';
...
export async function uploadChunks(
	...
	return trieveRequest<{ successful_chunks: number; failed_chunks: number }>('/api/v1/chunks', {
		method: 'POST',
		body: JSON.stringify({
...
export async function search(datasetId: string, query: string, limit = 5): Promise<SearchResult[]> {
 ...
 */
export async function ragRetrieve(query: string, top_k = 5): Promise<RagRetrieveResult[]> {
	// Get the configured dataset ID from env, or use a default
	const datasetId = process.env['TRIEVE_DEFAULT_DATASET_ID'];

	if (!datasetId) {
		throw new Error('TRIEVE_DEFAULT_DATASET_ID not set. Set it to your knowledge base dataset ID.');
	}

	const results = await search(datasetId, query, top_k);

	return results.map(
		(r: SearchResult): RagRetrieveResult => ({

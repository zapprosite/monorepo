...
export const webhookRouter = trpcRouter({
	...
	list: protectedProcedure
		...
		.query(async ({ input, ctx }) => {
			...
			const webhooks = await db.webhooks.where({ clienteId: input.clienteId }).select('*');
			return webhooks;
		}),
	...
	getById: protectedProcedure
		...
		.query(async ({ input, ctx }) => {
			const webhook = await db.webhooks.where({ id: input.id }).select('*').take();
	...
	update: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			const updated = await db.webhooks.where({ id: input.id }).update(input.data);
			return updated;
		}),
	...
	getDeliveryById: protectedProcedure
		.input(z.object({ id: z.string().uuid() }))
		.query(async ({ input, ctx }) => {
			const delivery = await db.webhookDeliveries.where({ id: input.id }).select('*').take();
			if (!delivery) throw new TRPCError({ code: 'NOT_FOUND' });
			// Verify delivery's webhook belongs to user's team
			const webhook = await db.webhooks.where({ id: delivery.webhookId }).take();
			if (!webhook) throw new TRPCError({ code: 'NOT_FOUND', message: 'Webhook não encontrado' });
			const client = await db.clients
				.where({ clientId: webhook.clienteId, teamId: ctx.user.teamId })
				// @ts-ignore TS2339
	...
	getEventoById: protectedProcedure
		.input(z.object({ id: z.string().uuid() }))
		.query(async ({ input, ctx }) => {
			const evento = await db.eventos.where({ id: input.id }).select('*').take();

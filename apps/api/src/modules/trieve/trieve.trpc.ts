...
export const trieveRouter = trpcRouter({
	...
	listDatasets: protectedProcedure.query(async ({ ctx }) => {
		...
		const teamMappings = await db.trieveDatasets.select('*').where({ teamId });
	...
	createDataset: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (input.isDefault) {
				await db.trieveDatasets.where({ teamId, isDefault: true }).update({ isDefault: false });
			}
	...
	ragRetrieve: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const results = await search(defaultDataset.trieveDatasetId, input.query, input.top_k);
			return results;
		}),
});

...
describe('mcp-connectors — auth guard', () => {
	...
	it('getById rejeita não autenticado', async () => {
		await expect(caller.mcpConectores.getById({ id: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});

	it('update rejeita não autenticado', async () => {
		await expect(caller.mcpConectores.update({ id: FAKE_UUID, data: {} })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});

	it('delete rejeita não autenticado', async () => {
		await expect(caller.mcpConectores.delete({ id: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});
...
describe('mcp-connectors — getById team isolation', () => {
	...
	it('getById lança FORBIDDEN para conector de outro team', async () => {
		const callerOther = createCaller(authContext({ teamId: 'team-other' }));
		await expect(callerOther.mcpConectores.getById({ id: FAKE_UUID })).rejects.toMatchObject({
			code: 'FORBIDDEN',
		});
	});

	it('getById lança erro para UUID inválido', async () => {
		await expect(caller.mcpConectores.getById({ id: 'not-a-uuid' })).rejects.toThrow();
	});
});
...
describe('mcp-connectors — delete team isolation', () => {
	...
	it('delete lança FORBIDDEN para conector de outro team', async () => {
		const callerOther = createCaller(authContext({ teamId: 'team-other' }));
		await expect(callerOther.mcpConectores.delete({ id: FAKE_UUID })).rejects.toMatchObject({
			code: 'FORBIDDEN',
		});
	});
});

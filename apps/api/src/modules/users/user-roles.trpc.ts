...
async function assertAdmin(userId: string): Promise<void> {
	const adminRole = await db.userRoles.where({ userId, role: 'Admin' }).takeOptional();
	if (!adminRole) {
		throw new TRPCError({
			code: 'FORBIDDEN',
...
export const userRolesRouterTrpc = trpcRouter({
	...
	revokeRole: protectedProcedure.input(userRoleRevokeZod).mutation(async ({ ctx, input }) => {
		...
		await db.userRoles.where({ userId: input.userId, role: input.role }).delete();
	}),
});

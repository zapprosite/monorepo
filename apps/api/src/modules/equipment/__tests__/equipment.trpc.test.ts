...
describe('equipment — auth guard (UNAUTHORIZED)', () => {
	...
	it('getUnitDetail rejeita não autenticado', async () => {
		await expect(caller.equipment.getUnitDetail({ unitId: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});
	...
	it('listEquipmentByUnit rejeita não autenticado', async () => {
		await expect(caller.equipment.listEquipmentByUnit({ unitId: FAKE_UUID })).rejects.toMatchObject(
			{ code: 'UNAUTHORIZED' },
		);
	});
...
describe('equipment — getUnitDetail validation', () => {
	...
	it('getUnitDetail lança erro para UUID inválido', async () => {
		await expect(caller.equipment.getUnitDetail({ unitId: 'not-a-uuid' })).rejects.toThrow();
	});
});
...
describe('equipment — listEquipmentByUnit validation', () => {
	...
	it('listEquipmentByUnit lança erro para UUID inválido', async () => {
		await expect(caller.equipment.listEquipmentByUnit({ unitId: 'not-a-uuid' })).rejects.toThrow();
	});
});

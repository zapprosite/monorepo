...
describe('content-engine — auth guard', () => {
	...
	it('list rejeita não autenticado', async () => {
		await expect(caller.conteudos.list({ clienteId: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});

	it('getById rejeita não autenticado', async () => {
		await expect(caller.conteudos.getById({ id: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});
...
describe('content-engine — list team isolation', () => {
	...
	it('list lança erro para UUID inválido em clienteId', async () => {
		await expect(caller.conteudos.list({ clienteId: 'not-a-uuid' })).rejects.toThrow();
	});
});
...
describe('content-engine — getById validation', () => {
	...
	it('getById lança erro para UUID inválido', async () => {
		await expect(caller.conteudos.getById({ id: 'not-a-uuid' })).rejects.toThrow();
	});
});

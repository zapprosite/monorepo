...
export const conteudoRouter = trpcRouter({
	// Conteúdos
	create: protectedProcedure.input(conteudoCreateInputZod).mutation(async ({ input, ctx }) => {
	...
	list: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			const conteudos = await db.conteudos
				.where({ teamId: ctx.user.teamId, clienteId: input.clienteId } as any)
				.select('*')
	...
	getById: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			const conteudo = await db.conteudos
				.where({ id: input.id, teamId: ctx.user.teamId } as any)
				.select('*')
				.take();
	...
	getBySlug: protectedProcedure.input(conteudoGetBySlugZod).query(async ({ ctx, input }) => {
		const conteudo = await db.conteudos
			.where({
				teamId: ctx.user.teamId,
				slug: input.slug,
	...
	delete: protectedProcedure
		.input(z.object({ id: z.string().uuid() }))
		.mutation(async ({ ctx, input }) => {
			const deleted = await db.conteudos.where({ id: input.id, teamId: ctx.user.teamId } as any).delete();
	...
	createRevisao: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const conteudo = await db.conteudos
				.where({ id: input.conteudoId, teamId: ctx.user.teamId } as any)
				.select('*')
				.take();
			if (!conteudo) throw new TRPCError({ code: 'NOT_FOUND' });
	...
	getRevisaoById: protectedProcedure
		.input(z.object({ id: z.string().uuid() }))
		.query(async ({ ctx, input }) => {
			const revisao = await db.conteudoRevisoes.where({ id: input.id }).select('*').take();
			if (!revisao) throw new TRPCError({ code: 'NOT_FOUND' });

			// Verify the parent conteudo belongs to team
			const conteudo = await db.conteudos
				.where({ id: revisao.conteudoId, teamId: ctx.user.teamId } as any)
				.select('*')
				.take();
	...
	updateRevisao: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const updated = await db.conteudoRevisoes.where({ id: input.id }).update(input.data);
			return updated;
		}),
	...
	deleteRevisao: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const _deleted = await db.conteudoRevisoes.where({ id: input.id }).delete();
			return { success: true };
		}),
});

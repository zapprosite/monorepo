...
export class KanbanCardsTable extends BaseTable {
	...
	columns = this.setColumns((t) => ({
		...
		prioridade: t.enum('kanban_card_priority_enum', KANBAN_CARD_PRIORITY_ENUM).default('Media'),
		status: t.enum('kanban_card_status_enum', KANBAN_CARD_STATUS_ENUM).default('Aberto'),
		responsavelId: t.uuid().nullable().foreignKey('users', 'userId', {
			onUpdate: 'RESTRICT',
			onDelete: 'SET NULL',
		}),

...
describe('kanban — auth guard (UNAUTHORIZED)', () => {
	...
	it('getBoardDetail rejeita não autenticado', async () => {
		await expect(caller.kanban.getBoardDetail({ boardId: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});

	it('createBoard rejeita não autenticado', async () => {
		await expect(caller.kanban.createBoard({ nome: 'Test', setor: 'TI' })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});

	it('updateBoard rejeita não autenticado', async () => {
		await expect(caller.kanban.updateBoard({ boardId: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});

	it('deleteBoard rejeita não autenticado', async () => {
		await expect(caller.kanban.deleteBoard({ boardId: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});
	...
	it('updateColumn rejeita não autenticado', async () => {
		await expect(caller.kanban.updateColumn({ columnId: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});

	it('deleteColumn rejeita não autenticado', async () => {
		await expect(caller.kanban.deleteColumn({ columnId: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});
	...
	it('getCardDetail rejeita não autenticado', async () => {
		await expect(caller.kanban.getCardDetail({ cardId: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});
	...
	it('updateCard rejeita não autenticado', async () => {
		await expect(caller.kanban.updateCard({ cardId: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});
	...
	it('deleteCard rejeita não autenticado', async () => {
		await expect(caller.kanban.deleteCard({ cardId: FAKE_UUID })).rejects.toMatchObject({
			code: 'UNAUTHORIZED',
		});
	});
});
...
describe('kanban — validação de input (Zod)', () => {
	...
	it('getBoardDetail rejeita boardId inválido', async () => {
		await expect(caller.kanban.getBoardDetail({ boardId: INVALID_UUID })).rejects.toThrow();
	});

	it('createBoard rejeita nome vazio', async () => {
		await expect(caller.kanban.createBoard({ nome: '', setor: 'TI' })).rejects.toThrow();
	});

	it('createBoard rejeita setor vazio', async () => {
		await expect(caller.kanban.createBoard({ nome: 'Board', setor: '' })).rejects.toThrow();
	});

	it('updateBoard rejeita boardId inválido', async () => {
		await expect(caller.kanban.updateBoard({ boardId: INVALID_UUID })).rejects.toThrow();
	});

	it('deleteBoard rejeita boardId inválido', async () => {
		await expect(caller.kanban.deleteBoard({ boardId: INVALID_UUID })).rejects.toThrow();
	});
	...
	it('updateColumn rejeita columnId inválido', async () => {
		await expect(caller.kanban.updateColumn({ columnId: INVALID_UUID })).rejects.toThrow();
	});

	it('deleteColumn rejeita columnId inválido', async () => {
		await expect(caller.kanban.deleteColumn({ columnId: INVALID_UUID })).rejects.toThrow();
	});
	...
	it('getCardDetail rejeita cardId inválido', async () => {
		await expect(caller.kanban.getCardDetail({ cardId: INVALID_UUID })).rejects.toThrow();
	});
	...
	it('createCard rejeita titulo vazio', async () => {
		await expect(caller.kanban.createCard({ columnId: FAKE_UUID, titulo: '' })).rejects.toThrow();
	});
	...
	it('updateCard rejeita cardId inválido', async () => {
		await expect(caller.kanban.updateCard({ cardId: INVALID_UUID })).rejects.toThrow();
	});
	...
	it('listCards rejeita columnId inválido', async () => {
		await expect(caller.kanban.listCards({ columnId: INVALID_UUID })).rejects.toThrow();
	});

	it('listCards rejeita boardId inválido', async () => {
		await expect(caller.kanban.listCards({ boardId: INVALID_UUID })).rejects.toThrow();
	});

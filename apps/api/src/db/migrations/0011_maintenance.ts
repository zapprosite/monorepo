...
change(async (db) => {
	...
	await db.createEnum('public.nivel_fidelidade', ['bronze', 'prata', 'ouro', 'platinum']);
	...
	await db.createTable(
		...
		(t) => [t.index(['clienteId']), t.index(['equipamentoId'])],
	);
	...
	await db.createTable(
		...
		(t) => ({
			id: t.uuid().primaryKey().default(t.sql`gen_random_uuid()`),
			planoManutencaoId: t.uuid().foreignKey('maintenance_plans', 'id', { onDelete: 'CASCADE' }),
			dataAgendada: t.timestamp(),
			statusManutencao: t.enum('status_manutencao').default('agendada'),
			tecnicoAtribuidoId: t.uuid().nullable().foreignKey('users', 'userId', {
				onUpdate: 'RESTRICT',
				onDelete: 'RESTRICT',
			}),
		...
		(t) => [t.index(['planoManutencaoId']), t.index(['dataAgendada'])],
	);
	...
	await db.createTable(
		...
		(t) => [t.index(['clienteId']), t.index(['statusReativacao'])],
	);
	...
	await db.createTable(
		...
		(t) => ({
			...
			templateId: t.uuid().nullable().foreignKey('email_templates', 'id', {
				onUpdate: 'RESTRICT',
				onDelete: 'SET NULL',
			}),

...
# ── docling conversion (used by hvac_add_manual.py) ─────────────────────────
def docling_convert(pdf_path: str, output_md_path: str) -> None:
    """
    Convert a PDF to Markdown using Docling (from the HVAC RAG venv).
    Writes the Markdown output to output_md_path.
    Raises RuntimeError if docling is unavailable.
    """
    from docling.document_converter import DocumentConverter, PdfFormatOption
    from docling.datamodel.base_models import InputFormat
    from docling.datamodel.pipeline_options_pdf import PdfPipelineOptions

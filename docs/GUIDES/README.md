...
- **Workflows** — How to execute multi-step processes (e.g., code review)
...
|----------|--------|---------|
| **SPEC** | What do we need to build? | "Service X needs a local STT proxy" |
| **ADR** | Why did we choose this approach? | "Why we use a specific STT/TTS approach" |
| **GUIDE** | How do we execute this operation? | "Run the health check to verify all services are healthy" |
...
**Naming:** `kebab-case.md` (e.g., `service-health-check.md`)

# SPEC-025 — Vitest Fixes
Corrigir Testes Vitest — Plano de Ação
...
3. **`haystack-integration.test.ts`** — Isolation failure ⚠️ REQUER REFATORAÇÃO
...
### 3. Test isolation failure em `haystack-integration.test.ts`
...
describe('LiteLLM Proxy', () => {
  ...
  it('should route to hermes-brain model', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      'http://localhost:4018/v1/chat/completions',
...
### Step 3: Corrigir `haystack-integration.test.ts` isolation

**Ficheiro:** `apps/hermes-gateway/src/__tests__/haystack-integration.test.ts`
...
| `haystack-integration.test.ts` | Isolation failure | P1 |
...
- O problema de isolation em `haystack-integration.test.ts` é um bug conhecido do Vitest quando múltiplos ficheiros de teste modificam globals partilhados.
...
- [ ] Corrigir isolation de `global.fetch` em `haystack-integration.test.ts`

...
Prune total do legado Hermes Agent de todos os SPECs, docs, skills, scripts e containers. HERMES é o único assistente. Criar pipeline de tasks e executar com 14 agentes em paralelo.

---

## Background

Auditoria de 15/04/2026 revelou salada de referências Hermes Agent legadas por todo o monorepo:

- SPECs arquivadas mas ainda referenciadas no SPEC-INDEX
- Skills com Hermes Agent em tabelas de configuração
- Containers Docker a correr mas sem propósito (Hermes Agent deprecated)
- Variáveis .env com chaves Hermes Agent ainda activas
...
**Decisão:** HERMES é o único assistente. Hermes Agent prune total.
...
### Ports to Prune (Hermes Agent legacy)

| Porta | Serviço                     | Acção             |
| ----- | --------------------------- | ----------------- |
| 4001  | Hermes Agent Bot                | REMOVER container |
...
| 4006  | mcp-monorepo (Hermes Agent MCP) | REMOVER container |
...
- SPEC-011 (Hermes Agent Agency Suite)
- SPEC-012 (Hermes Agent-update-discoverer)
- SPEC-019 (OpenWebUI Repair)
- SPEC-020 (OpenWebUI Hermes Agent Bridge)
- SPEC-025 (Hermes Agent CEO Mix Voice Stack)
- SPEC-026 (Hermes Agent TTS Route Fix)
- SPEC-009 (audio stack) → renomear para archived

### .env — Prune 12 variáveis

```
PRUNE: HERMES_AGENT_USER, HERMES_AGENT_PASSWORD, HERMES_AGENT_DEEPGRAM_API_KEY,
       HERMES_AGENT_GATEWAY_TOKEN, HERMES_AGENT_GEMINI_API_KEY,
       HERMES_AGENT_CONTAINER_NAME, HERMES_AGENT_FQDN,
       HERMES_AGENT_INTERNAL_URL, HERMES_AGENT_INTERNAL_PORT,
...
| Skill                                   | Acção                                                |
| --------------------------------------- | ---------------------------------------------------- |
| `coolify-access/SKILL.md`               | ARCHIVE (docker-compose Hermes Agent legacy)             |
| `coolify-sre/SKILL.md`                  | UPDATE (remover Hermes Agent/openwebui, manter wav2vec2) |
...
Hermes Agent-agents-kit/               (ARCHIVE dir inteiro)
```

### Smoke-tests — Delete 4 ficheiros

```
tasks/smoke-tests/pipeline-Hermes Agent-voice.sh    (DELETE)
...
smoke-tests/smoke-env-secrets-validate.sh        (DELETE — valida secrets Hermes Agent legadas)
```
...
tasks/plan-Hermes Agent-oauth-profiles.md
...
```
/srv/ops/scripts/Hermes Agent-to-hermes-migration.sh
/srv/ops/scripts/hermes-openwebui-setup.sh
/srv/ops/scripts/Hermes Agent-disable.sh
/srv/ops/scripts/hermes-migrate-now.sh (refere Hermes Agent container)
...
| `zappro-wav2vec2-proxy`  | 8203 | Deepgram proxy, Hermes Agent legacy   |
...
| `mcp-monorepo`           | 4006 | Hermes Agent MCP                      |
...
- [ ] SPEC-INDEX.md sem referências Hermes Agent (exceto arquivo)
- [ ] .env sem variáveis OPENCLAW\_
- [ ] docs/OPERATIONS/SKILLS/ sem ficheiros wav2vec2/openwebui/Hermes Agent
- [ ] tasks/smoke-tests/ sem scripts legacy
- [ ] Docker sem containers Hermes Agent legacy
- [ ] Skills actualizados sem Hermes Agent/openwebui

...
Haystack (8090) + Qdrant    → Search/RAG
Coolify (8000)            → Deploy
Gitea (3300/2222)         → Git
...
Searxng                   → Search
Edge TTS (8012)           → TTS
Qdrant (6333)             → DUPLICADO (tank + haystack + standalone)
...
- [ ] **1.3** Desduplicar Qdrant (manter só tank/qdrant + haystack-qdrant)
...
- Qdrant a correr em 3 lugares: tank/qdrant (ZFS), haystack-qdrant (docker), hermes-second-brain-qdrant (docker)

...
| M5 | Memory `.md` references stale gemma2 instead of llama3-portuguese-tomcat | Memory audit | memory/.md |
...
Also fix ARCHITECTURE-MODELS.md LiteLLM endpoint (:4018 not api.zappro.site)
...
Fix PORTS.md: Add 8202 (wav2vec2 host), 3457 (), 8050 (gotify), 8051 (alert-sender), 9080 (promtail). Fix wrong entries (8201 host mapping, 4003 nginx, 4007 tts-bridge)

Fix SERVICE_MAP.md: Remove supabase/caprover/old voice references. Update n8n (now in Coolify), (qwen2.5-vl), tts-bridge (UP on 8013 not REMOVED)
...
```
Files: memory/homelab-estado.md, memory/ai-context.md, memory/08-04-2026.md, memory/.md
...
Fix 08-04-2026.md: Annotate as superseded by 10-04-2026.md
Fix .md: Update gemma2 → llama3-portuguese-tomcat-8b-instruct-q8
...
Fix: Rewrite VOICE STACK section — current is wav2vec2 :8201 + :8880 via TTS Bridge :8013 + wav2vec2-deepgram-proxy :8203. Old (speaches:8010, chatterbox:8011) no longer exists.
...
### Cluster 8: .md fixes (H5, H6)
```
Files: docs/GOVERNANCE/.rules/.md, .claude/rules/.md
...
```
Files: docs/OPERATIONS/SKILLS/{docker-autoheal,node-exporter,loki,}.md (create)
...
.md: Document tasks/smoke-tests/.sh watchdog

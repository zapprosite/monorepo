...
- ✅ `GITEA_TOKEN` valid — `owner` (Gitea admin)
...
| `GITEA_TOKEN`                     | ✅ GitHub Actions (Gitea workflows)               | ACTIVE — `owner` admin (rotated)            |
...
| Token               | API Test                                            | Result                         |
| ------------------- | --------------------------------------------------- | ------------------------------ |
| `GITEA_TOKEN`       | `curl http://10.0.13.2:3000/api/v1/user` (internal) | ✅ **VALID** — `owner` (admin) |

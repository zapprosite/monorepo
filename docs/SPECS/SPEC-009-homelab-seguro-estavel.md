...
author: will-zappro
date: 2026-04-12
specRef: SPEC-002-homelab-control-plane.md, SPEC-AUDIT-FIXES-2026-04-12.md
---
...
45 2 * * * docker exec > /srv/backups/$(date +\%Y\%m\%d).sql
```
...
**.sh** executa a cada 5 minutos:
- Smoke test: TTS Bridge, , wav2vec2
- Auto-restart containers falhados
- Log em /srv/monorepo/logs//loop.log
...
**Verification:**
```bash
docker ps | grep -E "autoheal|tts-bridge|openwebui|"
crontab -l | grep 
```
...
- App ID: 99c85419
- Policy ID: 4a668d84
- Verify: `curl -sf -o /dev/null -w "%{http_code}" https://chat.zappro.site/` deve retornar auth challenge
...
- [ ] Store GH_TOKEN with `repo` scope in 
...
- SPEC-002-homelab-control-plane.md
...
7. GH_TOKEN stored in (PAT repo scope)

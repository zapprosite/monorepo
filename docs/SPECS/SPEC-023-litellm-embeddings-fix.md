...
# SPEC-023 — Litellm Embeddings Fix
LiteLLM Embeddings Hang Fix
...
- LiteLLM rodando em `127.0.0.1:4018` (bind local)
...
curl --max-time 15 -s -X POST "http://127.0.0.1:4018/v1/chat/completions" \
  -H "Content-Type: application/json" \
...
curl --max-time 15 -s -X POST "http://127.0.0.1:4018/v1/embeddings" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $LITELLM_MASTER_KEY" \
...
1. **NET-DEBUG**: Diagnosticar conectividade entre LiteLLM e Ollama

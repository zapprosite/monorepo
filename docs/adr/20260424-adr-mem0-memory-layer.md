...
|--------|-----------------|------------|
| **REPO** | Fonte de verdade — documentação versionada | Git + filesystem |
...
Historicamente, Qdrant era usado como store genérico para ambas as funções. Essa sobreposição criou ambiguidade sobre onde cada tipo de dado deveria residir. Além disso, a configuração do Mem0 estava quebrada — embedding model mismatch impedia retrieval.

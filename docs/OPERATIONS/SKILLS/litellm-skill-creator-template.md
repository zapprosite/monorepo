...
"Usa modelos LiteLLM para TTS (), STT (wav2vec2), Vision (qwen2.5-vl) e LLM (tom-cat-8b PT-BR)"

O skill deve poder:
- Sintetizar voz com (vozes pm_santa, pm_alex, pf_dora)
...
```bash
docker exec 6771lt8l7x8rqx72f sh -c 'mkdir -p /data/workspace/skills/lite-llm-voice && cat > /data/workspace/skills/lite-llm-voice/SKILL.md << "EOF"

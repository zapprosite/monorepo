...
curl -sf -m 10 -o /dev/null -w "%{http_code}" https://bot.zappro.site/ && echo " "

# Run smoke test
bash tasks/smoke-tests/pipeline-.sh 2>&1 | tail -10
...
docker ps -a --format "table {{.Names}}\t{{.Status}}" | grep -E "|litellm|wav2vec2|coolify"
...
# Example: check 
docker inspect 6771lt8l7x8rqx72f --format '{{json .NetworkSettings.Networks}}'
...
docker exec coolify-proxy curl -sf -m 5 http://6771lt8l7x8rqx72f:8080/health
```

...
**Purpose:** Automated monitoring and healing for homelab voice pipeline (+ LiteLLM + wav2vec2 + Traefik)
...
| `https://bot.zappro.site/` | 200 or 401 | |
...
| Pair | Required for |
|------|--------------|
| `coolify-proxy` <-> `*` | Traefik routing to |
...
```json
{
  "6771lt8l7x8rqx72f": {"count": 2, "last_attempt": "2026-04-08T14:30:00Z"},
...
```
2026-04-08T14:35:00Z INFO CONTAINER_OK 6771lt8l7x8rqx72f Status=running, Health=healthy
...
2026-04-08T14:50:00Z ALERT NETWORK_ISOLATED coolify-proxy<->6771lt8l7x8rqx72f No shared network - human intervention required
2026-04-08T14:55:00Z ALERT RATE_LIMITED 6771lt8l7x8rqx72f Max restart attempts (3) reached in last hour
```
...
```
2026-04-08T14:10:00Z WARN CONTAINER_DOWN 6771lt8l7x8rqx72f Status=exited
2026-04-08T14:10:00Z ALERT RATE_LIMITED 6771lt8l7x8rqx72f Max restart attempts (3) reached in last hour
```
...
1. Check container networks:
   ...
   docker inspect 6771lt8l7x8rqx72f --format '{{json .NetworkSettings.Networks}}' | python3 -m json.tool
   ```
...
- `INCIDENT-2026-04-08-.md` — Root cause analysis and prevention plan

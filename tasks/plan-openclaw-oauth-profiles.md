...
1. **Browser:** Chromium no container `browser-*` (coollabsio/)
...
3. `storageState` para operations já autenticado
...
│  1. Criar dirs /srv/data//{gemini,perplexity} │
...
│  5. Integrar com (credenciais GEMINI/PERPLEXITY) │
...
### Phase 1: Infrastructure
- [ ] **T1.1** Criar `/srv/data//{gemini,perplexity}` com perm 777
...
- [ ] **T2.2** Buscar credenciais GEMINI_EMAIL/TOKEN e PERPLEXITY_EMAIL/TOKEN do 
...
### Phase 3: 
- [ ] **T3.1** Configurar `BROWSER_STORAGE_STATE` ou similar
- [ ] **T3.2** Criar dois perfis/contextos no (gemini + perplexity)
- [ ] **T3.3** Testar que 

### Phase 4: Validation
- [ ] **T4.1** Restartar browser container, verificar que sessões persistem
- [ ] **T4.2** E2E: .zappro.site com conta Gemini
- [ ] **T4.3** E2E: .zappro.site com conta Perplexity
- [ ] **T4.4** Criar smoke test em `tests/.sh`

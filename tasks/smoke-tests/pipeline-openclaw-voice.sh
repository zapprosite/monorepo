...
# =============================================================================
# Load .env (has MINIMAX_API_KEY, TELEGRAM_BOT_TOKEN, etc.)
# =============================================================================
...
" "$model" "$prompt" "$max_tokens" 2>/dev/null)
    # Strip trailing /v1 from LITELLM_URL if present (avoids double-path when .env has https://llm.zappro.site/v1)
    local llm_base="${LITELLM_URL%/v1}"
    curl_post "${llm_base}/v1/chat/completions" "$LITELLM_KEY" "$json_payload"
}

...
1. 18789 para o host (Coolify networking)
2. Traefik não tem rota configurada para `.191.17.50.123.sslip.io`
3. Porta 8080 no host é o nginx do Coolify, não o 
...
Cloudflared Tunnel (ok ✅)
    │
    ▼
Traefik (:80/:443) ←————— PROBLEMA: não tem rota para .191.17.50.123.sslip.io
    │
    ▼ (mas deveria routear para)
(Coolify internal network)
```

O FQDN `6771lt8l7x8rqx72f.191.17.50.123.sslip.io` é gerado pelo Coolify/sslip.io, mas o Traefik não sabe routear este domínio para o container .
...
4. `curl -I https://.191.17.50.123.sslip.io/health` — o que acontece?
5. Verificar logs: `docker logs coolify-proxy --tail 50 | grep `

**Critério aceite:** Consegues listar todas as rotas Traefik e confirmar se 
...
Baseado nos templates existentes (, wav2vec2, litellm):
...
- Depois: `curl http://localhost:8090/api/http/routers | grep `
...
### Tarefa 6: Fix — Adicionar (se possível via Coolify)

**Método:** Via Coolify UI — expor 

Se Coolify gere Traefik dinamicamente:
1. No Coolify UI → → Settings → Add Domain
2. Adicionar: `.191.17.50.123.sslip.io`
3. Coolify automaticamente cria router no Traefik

**Critério aceite:** `curl -sf https://.191.17.50.123.sslip.io/health` retorna 200
...
```bash
# Deve retornar 200
curl -sf -m 10 "https://.191.17.50.123.sslip.io/health"

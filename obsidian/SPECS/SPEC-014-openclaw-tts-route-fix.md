# SPEC-014: — Corrigir Bridge + Voice
...
Corrigir a rota TTS do (`:8013`) com a voz correta (`pm_santa`). O problema atual: TTS Bridge está DOWN, , voz `pm_alex` não é filtrada.
...
|-------|---------------|-------------|
| TTS Route | `:8880` | TTS Bridge `:8013` |
...
### Grupo A — (10 agents)
1. Ver config real do container (docker exec cat /data/./.json)
2. Extrair TTS config path do source code 
...
8. Ver se existe mais de um container 
9. Listar networks do container (quais alcança )
...
- .md (aponta :8880 direto) → corrigir para :8013
- .md (aponta pm_alex) → corrigir para pm_santa
...
| AC-4 | | docker exec cat config |
| AC-5 | _santa | docker exec cat config |

...
│   └── guides/                # gitea-coolify, , security, etc.
│
...
| **Protected SPECs not machine-enforced** | MEDIUM | `SPEC-009-.md` marked PROTECTED but no hook prevents editing |
...
**SPEC PROTECTED status** (like `SPEC-009-.md`) is a flag meaning: "Changes require explicit human approval before merge." This is enforced by:
...
|----------|---------|----------|
| **SPEC** | Feature specification: what to build, user flows, API design, acceptance criteria | Voice pipeline, homelab network refactor, |
